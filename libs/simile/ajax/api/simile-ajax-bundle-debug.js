...
SimileAjax.ListenerQueue.prototype.fire = function (handlerName, args) {
    ...
    for (var i = 0;
        ...
        if (handlerName in listener) {
            ...
            if (this._wildcardHandlerName != null && this._wildcardHandlerName in listener) {
                try {
...
SimileAjax.EventIndex = function (unit) {
    ...
    this._events = new SimileAjax.SortedArray(function (event1, event2) {
...
SimileAjax.DateTime.roundUpToInterval = function (date, intervalUnit, timeZone, multiple, firstDayOfWeek) {
    ...
    if (date.getTime() < originalTime) {
        date.setTime(date.getTime() + SimileAjax.DateTime.gregorianUnitLengths[intervalUnit] * multiple);
    }
};
...
SimileAjax.DateTime.removeTimeZoneOffset = function (date, timeZone) {
    return new Date(date.getTime() + timeZone * SimileAjax.DateTime.gregorianUnitLengths[SimileAjax.DateTime.HOUR]);
};
...
SimileAjax.DOM.getEventRelativeCoordinates = function (evt, elmt) {
    if (SimileAjax.Platform.browser.isIE) {
        ...
        if ((evt.type == "DOMMouseScroll") && SimileAjax.Platform.browser.isFirefox && (SimileAjax.Platform.browser.majorVersion == 2)) {
            return{x: evt.screenX - coords.left, y: evt.screenY - coords.top};
        } else {
            return{x: evt.pageX - coords.left, y: evt.pageY - coords.top};
        }
    }
};
    ...
    outer:for (var i = 0;
        ...
        if (childNode.offsetWidth == 0 && childNode.offsetHeight == 0) {
            ...
            if (left <= x && top <= y && (x - left) < childNode.offsetWidth && (y - top) < childNode.offsetHeight) {
                ...
                if (childNode.nodeType == 1 && childNode.tagName == "TR") {
    ...
    if (templateNode == null) {
        ...
        if (typeof templateNode != "object") {
            ...
            if ("tag" in templateNode) {
                ...
                if (elmt == null) {
                    elmt = tag == "input" ? SimileAjax.DOM.createInputElement(templateNode.type) : document.createElement(tag);
                    if (parentElmt != null) {
                    ...
                    if (attribute == "className") {
                        ...
                        if (attribute == "id") {
                            ...
                            if (attribute == "title") {
                                ...
                                if (attribute == "type" && elmt.tagName == "input") {
                                } else {
                                    if (attribute == "style") {
                                        for (n in value) {
                                            var v = value[n];
                                        ...
                                        if (attribute == "children") {
                                            ...
                                            if (attribute != "tag" && attribute != "elmt") {
...
SimileAjax.Graphics._createTranslucentImageHTML1 = function (url, verticalAlign) {
    return'<img src="' + url + '"' + (verticalAlign != null ? ' style="vertical-align: ' + verticalAlign + ';"' : "") + " />";
};
SimileAjax.Graphics._createTranslucentImageHTML2 = function (url, verticalAlign) {
    var style = "width: 1px; height: 1px; filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + url + "', sizingMethod='image');" + (verticalAlign != null ? " vertical-align: " + verticalAlign + ";" : "");
...
SimileAjax.Graphics.createBubbleForPoint = function (pageX, pageY, contentWidth, contentHeight, orientation) {
    ...
    (function () {
        ...
        if (pageX - halfArrowGraphicWidth - bubbleConfig.borderGraphicSize - bubbleConfig.extraPadding > 0 && pageX + halfArrowGraphicWidth + bubbleConfig.borderGraphicSize + bubbleConfig.extraPadding < docWidth) {
            ...
            if ((orientation && orientation == "top") || (!orientation && (pageY - bubbleConfig.arrowGraphicTargetOffset - contentHeight - bubbleConfig.borderGraphicSize - bubbleConfig.extraPadding > 0))) {
                ...
                if ((orientation && orientation == "bottom") || (!orientation && (pageY + bubbleConfig.arrowGraphicTargetOffset + contentHeight + bubbleConfig.borderGraphicSize + bubbleConfig.extraPadding < docHeight))) {
        ...
        if ((orientation && orientation == "left") || (!orientation && (pageX - bubbleConfig.arrowGraphicTargetOffset - contentWidth - bubbleConfig.borderGraphicSize - bubbleConfig.extraPadding > 0))) {
...
SimileAjax.History.addLengthyAction = function (perform, undo, label) {
    SimileAjax.History.addAction({perform: perform, undo: undo, label: label, uiLayer: SimileAjax.WindowManager.getBaseLayer(), lengthy: true});
};
SimileAjax.History._handleIFrameOnLoad = function () {
    try {
        ...
        if (c < SimileAjax.History._currentIndex) {
            ...
            window.setTimeout(function () {
                while (SimileAjax.History._currentIndex > c && SimileAjax.History._currentIndex > SimileAjax.History._baseIndex) {
                    SimileAjax.History._currentIndex--;
            ...
            if (c > SimileAjax.History._currentIndex) {
                ...
                window.setTimeout(function () {
                    while (SimileAjax.History._currentIndex < c && SimileAjax.History._currentIndex - SimileAjax.History._baseIndex < SimileAjax.History._actions.length) {
                        var action = SimileAjax.History._actions[SimileAjax.History._currentIndex - SimileAjax.History._baseIndex];
                        try {
                            action.perform();
                        } catch (e) {
...
String.substitute = function (s, objects) {
    ...
    while (start < s.length - 1) {
        ...
        if (percent < 0 || percent == s.length - 1) {
            ...
            if (percent > start && s.charAt(percent - 1) == "\\") {
...
SimileAjax.WindowManager.registerEventWithObject = function (elmt, eventName, obj, handlerName, layer) {
    SimileAjax.WindowManager.registerEvent(elmt, eventName, function (elmt2, evt, target) {
        return obj[handlerName].call(obj, elmt2, evt, target);
    }, layer);
};
...
SimileAjax.WindowManager.registerForDragging = function (elmt, callback, layer) {
    SimileAjax.WindowManager.registerEvent(elmt, "mousedown", function (elmt, evt, target) {
...
SimileAjax.WindowManager.cancelPopups = function (evt) {
    ...
    while (i > 0 && SimileAjax.WindowManager._layers[i].ephemeral) {
        ...
        if (layer.elmt != null) {
            ...
            if (evtCoords.x >= elmtCoords.left && evtCoords.x < (elmtCoords.left + elmt.offsetWidth) && evtCoords.y >= elmtCoords.top && evtCoords.y < (elmtCoords.top + elmt.offsetHeight)) {
                break;
            }
        }
        i--;
...
SimileAjax.WindowManager._onBodyKeyDown = function (elmt, evt, target) {
    if (SimileAjax.WindowManager._dragging) {
        if (evt.keyCode == 27) {
            ...
            if ((evt.keyCode == 17 || evt.keyCode == 16) && SimileAjax.WindowManager._draggingMode != "copy") {
                ...
                if ("_ghostElmt" in callback) {
                    ...
                    if ("droppable" in callback && callback.droppable) {
                        ...
                        if (target != SimileAjax.WindowManager._potentialDropTarget) {
                            ...
                            var droppable = false;
                            if (target != null) {
                                if ((!("canDropOn" in callback) || callback.canDropOn(target)) && (!("canDrop" in target) || target.canDrop(SimileAjax.WindowManager._draggedElement))) {
                                    droppable = true;
                                }
                            }
                            if (droppable) {
