            ...
            var parseURLParameters = function (parameters) {
                ...
                for (var p = 0;
                    ...
                    if (pair[0] == "locales") {
                        ...
                        if (pair[0] == "defaultLocale") {
                            defaultServerLocale = pair[1];
                        } else {
                            if (pair[0] == "forceLocale") {
                                forceLocale = pair[1];
                                desiredLocales = desiredLocales.concat(pair[1].split(","));
                            } else {
                                if (pair[0] == "bundle") {
...
Timeline.CompactEventPainter.prototype.paintPreciseInstantEvent = function (evt, metrics, theme, highlightIndex) {
    var commonData = {tooltip: evt.getProperty("tooltip") || evt.getText()};
...
Timeline.HotZoneGregorianEtherPainter = function (params) {
    ...
    this._theme = params.theme;
    this._zones = [
        {startTime: Number.NEGATIVE_INFINITY, endTime: Number.POSITIVE_INFINITY, unit: params.unit, multiple: 1}
    ];
    for (var i = 0;
        ...
        for (var j = 0;
            ...
            if (zoneStart < zone2.endTime) {
                if (zoneStart > zone2.startTime) {
                    this._zones.splice(j, 0, {startTime: zone2.startTime, endTime: zoneStart, unit: zone2.unit, multiple: zone2.multiple});
                    j++;
                    zone2.startTime = zoneStart;
                }
                if (zoneEnd < zone2.endTime) {
                    this._zones.splice(j, 0, {startTime: zoneStart, endTime: zoneEnd, unit: zone.unit, multiple: (zone.multiple) ? zone.multiple : 1});
                    j++;
...
Timeline.YearCountEtherPainter.prototype.paint = function () {
    ...
    var labeller = {labelInterval: function (date, intervalUnit) {
        ...
        for (var j = 0;
            ...
            if (this._unit.compare(zoneStart, zone2.endTime) < 0) {
                if (this._unit.compare(zoneStart, zone2.startTime) > 0) {
                    this._zones.splice(j, 0, {startTime: zone2.startTime, endTime: zoneStart, magnify: zone2.magnify});
                    j++;
                    zone2.startTime = zoneStart;
                }
                if (this._unit.compare(zoneEnd, zone2.endTime) < 0) {
                    this._zones.splice(j, 0, {startTime: zoneStart, endTime: zoneEnd, magnify: zone.magnify * zone2.magnify});
                    j++;
                    zone2.startTime = zoneEnd;
                    zoneStart = zoneEnd;
                } else {
...
Timeline.OriginalEventPainter.prototype._getElClassName = function (elClassName, evt, prefix) {
    var evt_classname = evt.getClassName(), pieces = [];
    if (evt_classname) {
        if (prefix) {
...
Timeline._Impl.prototype.shiftOK = function (index, shift) {
    var going_back = shift > 0, going_forward = shift < 0;
...
(function () {
    ...
    if ("SimileWidgets_onLoad" in window) {
        if (typeof SimileWidgets_onLoad == "string") {
            ...
            if (typeof SimileWidgets_onLoad == "function") {
